...
Format all commits using Conventional Commits **with Korean descriptions**:
```
<type>: <한국어 설명>

[optional body - 한국어로 작성]
...
**Language Rules**:
- ✅ `type`: Use English (follows Conventional Commits standard)
- ✅ `description` and `body`: Write in Korean
- ✅ Keep emoji and attribution footer in original format

**Examples**:
```
feat: 네이버 블로그 콘텐츠 추출 기능 추가
fix: 데이터베이스 연결 타임아웃 오류 수정
docs: 설치 가이드 및 사용법 문서 업데이트
refactor: 환경 설정 로직 개선
test: AI 추출 기능 단위 테스트 추가
